def minimax (state, level, original, player, opponent, nodes):
	...
	for m in moves:
		...
		_, score, nodes = minimax(state, level-1, original, opponent, player, nodes)
...
def bestmove(state, level, player, opponent):
	...
	'''
	nodes = 0
	return minimax(state, level, player, player, opponent, nodes)
